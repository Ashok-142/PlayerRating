...
def _render_scoring_tab() -> None:
    ...
    default_index = match_ids.index(active_match_id) if active_match_id in match_ids else 0
    ...
    with live_tab:
        ...
        c1, c2 = st.columns(2)
        innings_no = int(c1.selectbox("Innings", options=[1, 2], key=f"live_innings_no_{match_id}"))
        batting_team_name = c2.radio(
            "Batting Team",
            options=[home_team_name, away_team_name],
            horizontal=True,
            key=f"live_batting_team_{match_id}",
        )

        if st.button("Load Innings", key=f"live_load_innings_{match_id}", use_container_width=True):
            try:
                ...
                innings_id = get_or_create_innings(
                    match_id=match_id,
                ...
                st.session_state.active_innings_id = innings_id
                st.success(f"Innings #{innings_no} loaded.")
                st.rerun()
            except Exception as exc:
                st.error(f"Could not load innings: {exc}")

        active_innings_id = st.session_state.get("active_innings_id")
        innings = get_innings(active_innings_id, DB_PATH) if active_innings_id else None
        if not innings or int(innings["match_id"]) != match_id:
            st.info("Load an innings to enable one-click scoring.")
            return
        ...
        with selector_col:
            ...
            if st.button("Swap Strikers", use_container_width=True, key=f"live_swap_{active_innings_id}"):
                state["striker_id"], state["non_striker_id"] = state["non_striker_id"], state["striker_id"]
                st.rerun()

            if state.get("needs_new_batter", False):
                st.warning(
                    "Wicket logged. Select a new batter by changing striker/non-striker, then continue."
                )
                last_out_player_id = state.get("last_out_player_id")
        ...
        with quick_col:
            st.caption("Quick Score Pad")
            run_actions = [("0", 0), ("1", 1), ("2", 2), ("3", 3), ("4", 4), ("6", 6)]
            run_cols = st.columns(len(run_actions))
            for col, (label, runs) in zip(run_cols, run_actions):
                if col.button(label, key=f"quick_run_{active_innings_id}_{runs}", use_container_width=True):
                    submit_action(runs_off_bat=runs, extras=0, extra_type="none")
            ...
            if extra_cols[4].button("Wicket", key=f"quick_wkt_{active_innings_id}", use_container_width=True):
                submit_action(
                    runs_off_bat=0,
                    extras=0,
                    extra_type="none",
            ...
            if c_undo.button("Undo Last Ball", key=f"quick_undo_{active_innings_id}", use_container_width=True):
                try:
            ...
            with st.expander("Advanced Event (rare cases)", expanded=False):
                c1, c2, c3 = st.columns(3)
                adv_runs = int(c1.number_input("Runs off bat", min_value=0, max_value=6, value=0, step=1))
                adv_extra_type = c2.selectbox(
                    "Extra Type",
                    options=["none", "wide", "no_ball", "bye", "leg_bye"],
                    format_func=lambda x: x.replace("_", " ").title(),
                )
                adv_extras = int(c3.number_input("Extras", min_value=0, max_value=6, value=0, step=1))
                adv_is_wicket = st.checkbox("Wicket")
                adv_dismissal = None
                adv_out_player_id = None
                if adv_is_wicket:
                    w1, w2 = st.columns(2)
                    adv_dismissal = w1.selectbox(
                        "Dismissal",
                        options=[
                            "bowled",
                            "caught",
                    ...
                    out_row = w2.selectbox(
                        "Out Player",
                        options=batting_players,
                        format_func=_player_option,
                    )
                    adv_out_player_id = int(out_row["player_id"])
                adv_notes = st.text_input("Notes", key=f"adv_notes_{active_innings_id}")
                if st.button("Save Advanced Event", key=f"adv_save_{active_innings_id}", use_container_width=True):
                    submit_action(
                        runs_off_bat=adv_runs,
                        extras=adv_extras,
                        extra_type=adv_extra_type,
                        is_wicket=adv_is_wicket,
                        dismissal_type=adv_dismissal,
                        dismissed_player_id=adv_out_player_id,
                        notes=adv_notes,
                    )
...
def main() -> None:
    ...
    try:
        ...
        raw_df = _records_to_dataframe(records)
        if not records:
